...
const useGasFee = (selectedToken, needApproval) => {
  ...
  const calculateGasFee = useCallback(async () => {
    if (
      ...
      if (fromNetwork.isL1) {
        const { maxFeePerGas, maxPriorityFeePerGas } = await getPublicClient(defaultConfig, { chainId: fromNetwork.chainId })!.estimateFeesPerGas()
        gasPrice = maxFeePerGas as bigint
        priorityFee = maxPriorityFeePerGas as bigint
      } else {
        const { gasPrice: legacyGasPrice } = await getPublicClient(defaultConfig, { chainId: fromNetwork.chainId })!.estimateFeesPerGas({
          type: "legacy",
        })
